suppressPackageStartupMessages({
  library(shiny); library(shinythemes); library(readxl); library(dplyr); library(tidyr)
  library(stringr); library(ggplot2); library(ggrepel); library(pheatmap); library(DT)
})
...
build_plot_table <- function(stats_df, annotation_df) {
  ...
  label_col <- NULL
  for (nm in c("Name", "Compound Name", "Compound", "Feature_Label")) if (nm %in% names(out)) { label_col <- nm; break }
...
save_heatmap <- function(heatmap_mat, meta, out_file, group_colors) {
  ...
  pheatmap::pheatmap(heatmap_mat, color = colorRampPalette(c("#6BAED6", "#F7F7F7", "#CB181D"))(100),
    ...
    border_color = "grey65", fontsize = 10, fontsize_row = 9, fontsize_col = 10, angle_col = 270,
    treeheight_row = 55, treeheight_col = 55, cellwidth = 28, cellheight = 15,
...
plot_individual_bar <- function(feature_name, label_name, mat, meta, group_a, group_b, group_a_color, group_b_color, out_file) {
  ...
  p <- ggplot(df, aes(x = group, y = value, fill = group)) +
    ...
    theme(plot.title = element_text(face = "bold", size = 14, margin = margin(b = 8)), plot.subtitle = element_text(size = 11, margin = margin(b = 12)), panel.grid.minor = element_blank(), legend.position = "none") +
...
img_panel <- function(title, src, note = NULL) {
  tagList(
    tags$div(style = "font-weight:700;margin:0 0 10px 0;", title),
    if (!is.null(note)) tags$div(style = "color:#666;margin:0 0 10px 0;", note),
    tags$img(src = src, style = "width:100%;height:auto;display:block;border:1px solid #e5e5e5;border-radius:6px;background:#fff;")
  )
}

ui <- fluidPage(
  theme = shinytheme("flatly"),
  tags$head(tags$style(HTML(".plot-card{width:100%;overflow-x:auto}.barplot-card{margin-bottom:22px;border:1px solid #ddd;border-radius:8px;padding:10px;background:#fff}.barplot-card img{width:100%;height:auto;display:block}.tab-content{overflow-x:hidden}"))),
  titlePanel("Compound Discoverer Metabolomics Explorer"),
  sidebarLayout(
    sidebarPanel(
      fileInput("xlsx", "Upload Compound Discoverer Excel file", accept = c(".xlsx", ".xls")),
      uiOutput("sheet_ui"), tags$hr(),
      ...
      tags$hr(), downloadButton("download_zip", "Download Results ZIP")
    ),
    mainPanel(tabsetPanel(
      tabPanel("Preview", h4("Detected samples"), DTOutput("sample_table"), h4("Metadata used"), DTOutput("meta_table")),
      tabPanel("PCA", uiOutput("pca_ui")),
      tabPanel("Volcano", uiOutput("volcano_ui")),
      tabPanel("Heatmap", uiOutput("heatmap_ui")),
      tabPanel("Sample summaries", uiOutput("tic_ui"), br(), uiOutput("detect_ui"), br(), uiOutput("median_ui")),
...
server <- function(input, output, session) {
  ...
  parsed_data <- eventReactive(list(input$xlsx, input$sheet), { req(input$xlsx, input$sheet); read_cd_file(input$xlsx$datapath, sheet = input$sheet) }, ignoreNULL = FALSE)
  metadata_used <- reactive({
    ...
    if (!is.null(input$metadata)) {
      ...
      md$sample <- sanitize_name(md$sample); md$group <- as.character(md$group); md
    } else infer_groups(cd$sample_map$sample, input$group_a, input$group_b, input$pattern_a, input$pattern_b)
  })
  observe({ cd <- try(parsed_data(), silent = TRUE); if (!inherits(cd, "try-error")) output$sample_table <- renderDT(datatable(cd$sample_map, options = list(scrollX = TRUE, pageLength = 15))) })
  observe({ md <- try(metadata_used(), silent = TRUE); if (!inherits(md, "try-error")) output$meta_table <- renderDT(datatable(md, options = list(scrollX = TRUE, pageLength = 15))) })
  ...
  analysis <- eventReactive(input$run, {
    withProgress(message = "Running analysis", value = 0, {
      ...
      stats_df <- compute_stats(cd$matrix, meta, input$group_a, input$group_b); plot_df <- build_plot_table(stats_df, cd$annotation)
      ...
      top_for_heatmap <- head(top_for_heatmap, input$heatmap_top_n); heatmap_features <- unique(top_for_heatmap$Feature_ID)
      hm_meta <- meta
      ...
      outdir <- file.path(tempdir(), paste0("cd_shiny_", format(Sys.time(), "%Y%m%d_%H%M%S"))); dir.create(outdir, recursive = TRUE, showWarnings = FALSE); dir.create(file.path(outdir, "barplots"), showWarnings = FALSE)
      resource_prefix <- paste0("results_", as.integer(Sys.time())); addResourcePath(resource_prefix, outdir)
      ...
      incProgress(0.10, detail = "Rendering volcano plot")
      ...
      incProgress(0.10, detail = "Rendering sample summary plots")
      tic_vals <- colSums(cd$matrix[, meta$sample, drop = FALSE], na.rm = TRUE); det_vals <- colSums(cd$matrix[, meta$sample, drop = FALSE] > 0, na.rm = TRUE); med_vals <- apply(cd$matrix[, meta$sample, drop = FALSE], 2, median, na.rm = TRUE)
      ...
      incProgress(0.10, detail = "Rendering individual bar plots")
      ...
      if (nrow(top_bar) > 0) for (i in seq_len(nrow(top_bar))) {
        feature_id <- top_bar$Feature_ID[i]; label <- top_bar$Label[i]; safe_name <- gsub("[^A-Za-z0-9_\\-]+", "_", label)
      ...
      list(outdir = outdir, plot_df = plot_df, top_bar = top_bar)
    })
  })

  output$pca_ui <- renderUI({ req(analysis(), rv$resource_prefix); tags$div(class = "plot-card", img_panel("PCA", sprintf("/%s/pca.png", rv$resource_prefix))) })
  output$volcano_ui <- renderUI({ req(analysis(), rv$resource_prefix); tags$div(class = "plot-card", img_panel("Volcano Plot", sprintf("/%s/volcano.png", rv$resource_prefix))) })
  output$heatmap_ui <- renderUI({ req(analysis(), rv$resource_prefix); tags$div(class = "plot-card", img_panel("Clustered Heatmap", sprintf("/%s/heatmap.png", rv$resource_prefix), "Row z-scores with hierarchical clustering and class legend.")) })
  ...
  output$barplot_gallery <- renderUI({
    ...
    tagList(lapply(files, function(f) tags$div(class = "barplot-card", tags$img(src = sprintf("/%s/barplots/%s", rv$resource_prefix, utils::URLencode(f, reserved = TRUE))))))
  })
  output$download_zip <- downloadHandler(
    filename = function() paste0("cd_shiny_results_", format(Sys.time(), "%Y%m%d_%H%M%S"), ".zip"),
